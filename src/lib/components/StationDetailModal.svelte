<script lang="ts">
	...
	const open = $derived(loading || detail !== null);
		...
		<div class="w-full sm:max-w-lg bg-surface-900 border border-white/10 sm:rounded-xl overflow-hidden shadow-2xl max-h-[90dvh] flex flex-col">
			{#if loading}
				...
				<div class="p-6 animate-pulse space-y-4">
					<div class="h-5 bg-surface-700 rounded w-2/3"></div>
					<div class="h-3 bg-surface-700 rounded w-1/3"></div>
			...
			{:else if detail}
				<!-- Header -->
				<div class="flex items-start justify-between p-4 border-b border-white/8">
					<div>
				...
				<div class="overflow-y-auto flex-1 p-4 space-y-5">
					...
					<div>
						...
						<p class="text-sm text-surface-200">
							{detail.Morada.Morada}, {detail.Morada.Localidade}<br />
